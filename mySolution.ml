...
type com =
  ...
  | Tuple of int
  | Get of int 
  | Fun_Mutual of (var * var * prog) list
  | Return
  | Call
...
let com_arity (c : com): int =
  ...
  | Pop  | Neg | Not | Local(_) | Global(_) | InjL | InjR | CaseLeftRight(_)| Tuple(_) | Get(_)-> 1
...
let rec string_of_value v =
  ...
  | VString(s) -> "\"" ^ s ^ "\""
  | VRight(givenVal)  -> "Right " ^ string_of_value(givenVal)
  | VLeft (givenVal)  -> "Left " ^ string_of_value(givenVal)
  | VTuple (givenVal) -> 
    "(" ^
    makeTuple(givenVal) ^ ")"
  | VClosure ((Var(funcName), Var(varName),_ ), _, _) -> "Clo " ^ "(" ^ funcName ^ ", " ^ varName ^ ")"
and makeTuple (givenVal: 'a list): string = 
    let (genTuple, n) = 
    (List.fold_left
        (
        fun (acc, n) elem -> if n = 1 then (acc^string_of_value(elem), n) else (acc^string_of_value(elem)^", ", n-1)
        ) ("", List.length(givenVal)) givenVal) in 
      genTuple
...
let rec string_of_com (c : com) : string =
  ...
  | InjL      ->  "InjL"
  | InjR      ->  "InjR"
  | Tuple(n)  -> Printf.sprintf "Tuple %s" (string_of_int n)
  | Get(i)    -> Printf.sprintf "Get %s" (string_of_int i)
  | _ -> "failwidth" 
...
let rec parse_com line_num ls : (com * int * token list, parse_err) result =
 ...
 (*Parsing the inject right*)
  | SYMBOL("Tuple")::NUMBER(fst)::rst -> Ok(Tuple(fst), line_num, rst)
  | SYMBOL("Tuple")::fst::rst -> 
    Error(ExpectedVar(line_num, fst))

  | SYMBOL("Get")::NUMBER(fst)::rst -> Ok(Get(fst), line_num, rst)
  | SYMBOL("Get")::fst::rst ->
    Error(ExpectedVar(line_num, fst))
  
  | SYMBOL("Fun")::rst ->
    parse_functionslist line_num rst
    |> and_then @@ fun (fList, line_num, rst) -> Ok(Fun_Mutual(fList), line_num, rst)
...
and parse_functionslist (line_num: int) (ls: token list): 
    ...
    | SYMBOL(funcName)::SYMBOL(varName)::rst -> 
      ...
      make_var line_num varName  |> and_then @@ fun f2 ->
        ...
        |> and_then @@ fun (line_num, rst) ->
          (
            match (parse_com_list line_num (SYMBOL("Mut")) rst, parse_com_list line_num (SYMBOL("End")) rst) with 
            | Ok(prog1, line_num1, rst1), _ -> 
                parse_functionslist line_num1 rst1
                |> and_then @@ fun(fList, line_num, rst) ->
                  Ok((f, f2, prog1)::fList, line_num, rst)
            | _, Ok(prog, line_num, rst)      -> Ok((f, f2, prog)::[], line_num, rst)
            | Error(_), _ -> Error(UnexpectedEOF(line_num))
          )
...
(*-------------------------------------------------*)
let funMutEval (s, l, loc, glob: state) (f: (var*var*prog) list): (state, eval_err) result = 
  let newEnv = 
  List.fold_left
  (
    fun acc (funcName, varName, prog) -> add_bind (funcName) (VClosure  ((funcName, varName, prog), loc, f)) acc
  ) loc f 
...
let rec popNTimes (n: int) (s, l, loc, glob: state) = 
  match (s, n) with 
  | [], 0 -> Ok(s)
  | [], _ -> Error(InvalidArity(Pop, n))
  | h::t, num when num > 0  -> popNTimes (n-1) (t, l, loc, glob)
  | h::t, 0   -> Ok(s)
  | _, _ -> Error(InvalidArity(Pop, n))
(*-------------------------------------------------*)
let tuple (n: int) (s, l, loc, glob : state): (state, eval_err) result = 
  if n = 0 then 
    Ok((VTuple([])::s, l, loc, glob))
else 
    match List.nth_opt (s) (n-1) with 
    | None    -> Error(InvalidArity(Tuple(n), 0))
    | Some(v) -> 
        let (tupleArr, num) = 
          List.fold_left
          (
            fun (acc, num) elem -> 
              if num > 0 then
              (match (acc, num) with 
              | (_, someVal) -> (elem::acc, someVal-1) 
              )
              else 
                (acc, num)
          ) ([], n) s
          in 
          match (tupleArr, num) with 
          | h::t, 0 -> 
              (
              match popNTimes (n) (s, l, loc, glob) with 
              | Ok(stk)   -> Ok((VTuple(tupleArr)::stk, l, loc, glob))
              | _         -> Error(NoValue(Tuple(n)))
              )
          | _, _    -> Error(NoValue(Tuple(n)))
(*-------------------------------------------------*)
let get (i: int) (s, l, loc, glob : state): (state, eval_err) result = 
  ...
  | VTuple(lst)::rst ->
    ...
    | None    ->  Error(InvalidArity(Tuple(i), 0))
...
(*-------------------------------------------------*)
let rec begin_end (s, l, loc, glob : state) (blk : prog): (bool * bool*state, eval_err) result =
...
(*-------------------------------------------------*)
and ifthen_else (s, l, loc, glob : state) (then_blk : prog) (else_blk : prog): (bool * bool* state, eval_err) result =
  match s with
  | VInt(v)::rst when v = 1
...
and caseLeftRight (s, l, loc, glob: state) (left: prog) (right: prog): (bool* bool* state, eval_err) result =
    match s with 
    | VLeft(fst) ::rst   -> 
...
and call (s, l, loc, glob: state) = 
    ...
    |  VClosure((f, arg, prog), loc, funmutLst)::x::rst ->
        ...
        | Ok(_, _, newLoc, _) -> eval prog ([], l, add_bind arg x newLoc, glob ) |> 
...
and eval (prog : prog) (st : state) : (bool * bool* state, eval_err) result =
  ...
  | BeginEnd(p)      :: prog -> begin_end st p
                                ...
                                if not quitting && not returning then eval prog st else Ok(quitting, returning, st)
  | IfThenElse(t, e) :: prog -> ifthen_else st t e
                                |> and_then @@ fun (quitting, returning, st) ->
                                if not quitting && not returning then eval prog st else Ok(quitting, returning, st)
  | InjL      :: prog     -> injl st    |> and_then (eval prog)
  | InjR      :: prog     -> injr st    |> and_then (eval prog)
  | CaseLeftRight(l, r) :: prog -> caseLeftRight st l r
                                |> and_then @@ fun (quitting, returning, st) ->
                                if not quitting && not returning then eval prog st else Ok(quitting, returning, st)
 | Tuple(n)  :: prog      -> tuple n st      |> and_then (eval prog)
 | Get(i)    :: prog      -> get i st        |> and_then (eval prog)
 | Fun_Mutual(f) :: prog  -> funMutEval st f |> and_then (eval prog)
 | Call:: prog            -> call st         |> and_then @@ fun(quitting, returning, st) ->
                                            if not quitting then eval prog st else Ok(quitting, returning, st)
 | Return :: prog -> Ok(false, true, st)
